...
# Calls all shared infrastructure modules for the staging environment.
#
...
module "vpc" {
  ...
  single_nat_gateway        = false # Dual NAT GW in staging
  enable_flow_logs          = true  # Enable Flow Logs in staging
...
# ─────────────────────────────────────────────
# Module: RDS PostgreSQL (Multi-AZ in Staging)
# ─────────────────────────────────────────────
module "rds" {
  source = "../../modules/rds"
  ...
  db_username             = var.db_username
  db_password             = var.db_password
  multi_az                = true # Multi-AZ in staging
