...
# Calls all shared modules to provision the full infrastructure stack
...
module "secrets_manager" {
  ...
  service_secrets = {
    "service-a" = {
      INTERNAL_API_KEY = var.service_a_api_key
    }
    "service-b" = {
      INTERNAL_API_KEY = var.service_b_api_key
    }
    "service-c" = {
      INTERNAL_API_KEY = var.service_c_api_key
    }
  }
...
module "rds" {
  ...
  db_username             = var.db_username
  db_password             = var.db_password
  multi_az                = false      # Single AZ in dev for cost
  backup_retention_period = 3
  tags                    = local.common_tags
}

# ─────────────────────────────────────────────
# Module: ECS Service – React Frontend
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# Module: ECS Service – Microservice A (Java)
# ─────────────────────────────────────────────
module "ecs_service_a" {
  ...
  secrets = [
    {
      name       = "DB_CREDENTIALS"
      value_from = module.secrets_manager.db_credentials_secret_arn
    }
  ]
...
# ─────────────────────────────────────────────
# Module: ECS Service – Microservice B (Java)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# Module: ECS Service – Microservice C (Java)
# ─────────────────────────────────────────────
